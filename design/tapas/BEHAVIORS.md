...
[Explore the signal and its transformation in HTML](dictado-motion.html). El borde is selected for Dictado: a 124 × 28 px top-edge tab, small enough to remain unobtrusive during a short take. It always labels its recording state. The larger cards and pill are no longer presented as competing Dictado directions. This choice is documented in the design reference; the native app still uses the previously implemented compact panel.
