...
### Small recording signals and motion study

[Compare the three treatments in HTML](dictado-motion.html). The larger popup cards were replaced after review: the recording indicator should occupy much less space and need not be a window. La miga is a 188 × 40 px bottom-center pill. El pintxo, the leading exploration, is an 82 × 78 px floating mark at the lower right with no enclosing panel. El borde is a 124 × 28 px top-edge tab. Dimensions describe the recording control at actual CSS size, excluding optional captions and secondary controls. These are review concepts, not native settings. Idle remains visible in the study to make the controls discoverable; production visibility still needs a final decision.

The signal always labels its state. Clicking it starts or finishes a take; hover or keyboard focus reveals Finish and Cancel controls. Escape also cancels. Live words default off in the study and appear in a separate two-line caption when enabled. The caption follows the latest words; complete text remains available for delivery. Neither live words nor their arrival resizes the recording control.

The pintxo opens while listening, gathers during finalization, and settles on delivery. A delivery receipt holds for 1.6 seconds before idle; recovery stays until resolved and may use a larger, readable surface. Quiet motion and the OS reduced-motion preference remove animated entrances and ingredient movement while retaining state labels. In production, level and ingredient movement must follow actual audio, not the study's scripted animation. The native signal should stay on the display where the take began, preserve destination focus, and clear the Dock or menu bar. Do not move it in response to individual words.
